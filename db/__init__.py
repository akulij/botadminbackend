...
class DB:
    ...
    async def get_new_users_total(self, botname: str, days: int = 5):
        ...
        maxdate = datetime.datetime.combine(datetime.date.today(), datetime.datetime.min.time())
        q = select(ActionV1).where(ActionV1.bot == botname).where(ActionV1.actiontype == "register")
        e = await self._exec_query(q)
        actions: list[ActionV1] = e.all()
        ...
        for _ in range(days - 1):
            print(f"{maxdate=}", flush=True)
            q = select(ActionV1).where(ActionV1.bot == botname).where(ActionV1.actiontype == "register").where(ActionV1.time < maxdate)
            e = await self._exec_query(q)
            actions: list[ActionV1] = e.all()
